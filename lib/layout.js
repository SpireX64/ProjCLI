...
const SUBDIRS = ['repo', 'worktrees', 'notes', 'tasks', path.join('tasks', 'archive'), path.join('tasks', 'backlog'), 'etc'];

/**
 * @param {string} destRoot
 * @param {{ skipRepo?: boolean }} [opts]
 */
export function createStandardLayout(destRoot, opts = {}) {
  fs.mkdirSync(destRoot, { recursive: true });
  for (const s of SUBDIRS) {
    if (opts.skipRepo && s === 'repo') continue;
