...
import { loadTags, validateTagsAgainstRegistry, ensureTagsInRegistry } from './tags.js';
  ...
  if (ids.length === 0) {
    console.warn(
      'Warning: .tags is empty or missing; tags are not validated against a registry. ' +
        'Register tag ids with: proj tag <id> [description...]'
    );
  }
}
...
export async function collectInteractive(root, defaults = {}) {
  ...
  try {
    ...
    ensureTagsInRegistry(root, tags);
