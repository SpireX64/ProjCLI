...
/**
 * Write or merge minimal .projrc: created, optional name, optional owner.
 * @param {string} projectAbs
 * @param {{ projectName: string, owner?: string }} meta
 */
export function writeInitialProjrc(projectAbs, meta) {
  const created = computeCreatedTimestamp(projectAbs);
  /** @type {Map<string, string>} */
  const map = new Map();
