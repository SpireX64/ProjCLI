...
function buildStatusDetailJson(root, e) {
  ...
  const rp = repoPathForProject(e.abs, e.parsed.projectName);
  const worktreesRaw = hasGitRepo(rp)
    ? filterWorktreesInLayout(e.abs, e.parsed.projectName, listWorktreesDetail(rp))
    : [];
  const worktrees = worktreesRaw.map((w) => ({
    path: path.relative(e.abs, w.path),
    branch: w.branch,
    dirty: w.dirty,
  }));
  const skip = new Set(['name', 'owner', 'created', 'tags']);
  const projrc = Object.fromEntries(
      ...
      for (const w of wts) {
        const wt = w.dirty
          ...
          : colored
            ? color.green('clean')
            : 'clean';
        const wtName = path.basename(w.path);
        console.log(`  ${wtName}\t${w.branch || '?'}\t${wt}`);
      }
    } else {
