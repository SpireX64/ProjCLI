...
export async function cmdNew(rest) {
  ...
  if (hint) {
    ...
    if (!exists && validateFolderName(base)) {
      ...
      assertTagsAllowed(root, parsed.tags);
    ...
    if (!exists) {
      ...
      const canonical = buildFolderName(data.own, data.type, data.projectName, data.tags);
      ...
      createStandardLayout(dest);
      writeInitialProjrc(dest, { projectName: data.projectName, owner: data.owner });
      console.log(dest);
      return;
    }
