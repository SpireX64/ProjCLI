...
import { ensureMainWorktreeDir, mainWorktreePath } from '../worktrees.js';
...
export async function cmdNew(rest) {
  ...
  if (hint) {
    ...
    if (!exists && validateFolderName(base)) {
      ...
      ensureMainWorktreeDir(dest, parsed.projectName);
    ...
    if (!exists) {
      ...
      createStandardLayout(dest);
      ensureMainWorktreeDir(dest, data.projectName);
    ...
    const canonical = buildFolderName(data.own, data.type, data.projectName);
    const dest = path.join(root, canonical);
    const destMain = mainWorktreePath(dest, data.projectName);
    ...
    if (dry) {
      console.log(
        `Would create ${dest}, move all contents from ${sourceAbs} into ${destMain}, delete source (has .git: ${hasGit})`
      );
      return;
    }
    if (!stdinIsTTY() && !force) {
    ...
    if (stdinIsTTY() && !force) {
      const ok = await confirmImportSourceRemoval(sourceAbs, destMain);
      if (!ok) {
    ...
    createStandardLayout(dest);
    moveDirectoryContents(sourceAbs, destMain);
    try {
