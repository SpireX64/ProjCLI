...
import { syncWorktreesAfterProjectRename } from '../worktrees-sync.js';
...
export async function cmdMv(rest) {
  ...
  if (rest.length === 2) {
    ...
    const parsedFrom = parseFolderName(from);
    if (!parsedFrom) throw new Error('Invalid source');
    ...
    syncWorktreesAfterProjectRename(toPath, parsedFrom.projectName, parsedTo.projectName);
  ...
  try {
    ...
    syncWorktreesAfterProjectRename(dest, cur.projectName, pn);
