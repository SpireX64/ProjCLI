...
import { resolveProject, isProjectNameTaken } from '../projects.js';
import { relinkFavoritesAfterRename } from '../fav.js';
import { createPrompt, warnTagRegistry, assertTagsAllowed } from '../cli-shared.js';
...
export async function cmdMv(rest) {
  ...
  if (rest.length === 2) {
    ...
    assertTagsAllowed(root, parsedTo.tags);
  ...
  try {
    ...
    const tagsRaw = await p.askKeep('Tags (comma-separated)', cur.tags.join(',') || '');
    ...
    const newBase = buildFolderName(own, type, pn, tags);
