 ...
 * @param {string[]} tags from `.projrc`
 * @returns {string} e.g. `MyApp (personal application) — web, rn`
 */
export function formatProjectLine(parsed, tags) {
 ...
 * @param {string[]} tags from `.projrc`
 */
export function formatProjectLineColor(parsed, tags) {
  if (!useColor()) return formatProjectLine(parsed, tags);
  const kind = describeOwnType(parsed.own, parsed.type);
